...
export const ProductModal: React.FC<ProductModalProps> = ({ product, onClose }) => {
  ...
  const handleContact = () => {
    const message = `مرحبا، أود الاستفسار عن المنتج: ${product.name}`;
          ...
          <div className="md:w-1/2 p-8 md:p-12 overflow-y-auto">
            ...
            {STORE_CONFIG.showPrices && displayPrice && (
              <div className="flex items-center gap-4 mb-8">
                ...
                <span className="px-2 py-1 bg-accent/10 text-accent text-[10px] font-bold rounded uppercase">
                  {STORE_CONFIG.indicativePriceText}
                </span>
              </div>
            )}
            ...
            <div className="space-y-6 mb-10">
              <div className="space-y-2">
                <label className="text-sm font-bold text-primary block">وصف المنتج</label>
                <p className="text-stone-500 leading-relaxed">
                  للمزيد من التفاصيل حول هذا المنتج والمواصفات الفنية، يرجى التواصل معنا مباشرة. نحن نوفر حلولاً متكاملة لقطاع المطاعم والمقاهي والفنادق.
                </p>
              </div>
            </div>
            ...
            <div className="mt-8 pt-8 border-t border-stone-100 flex items-start gap-3 text-stone-400">
              ...
              <p className="text-xs leading-relaxed">
                نحن في {STORE_CONFIG.name} نلتزم بتوفير أجود المنتجات الغذائية لشركائنا في قطاع الضيافة والأغذية.
              </p>
            </div>
          </div>
        </motion.div>
