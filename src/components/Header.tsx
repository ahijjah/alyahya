      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 md:h-20 gap-4">
          ...
          <nav className="hidden md:flex items-center space-x-reverse space-x-8">
            <Link to="/" className={`text-sm font-bold transition-colors hover:text-accent ${location.pathname === '/' ? 'text-accent' : 'text-espresso'}`}>الرئيسية</Link>
              ...
              <AnimatePresence>
                {isCatDropdownOpen && (
                  <motion.div 
                    ...
                    className="absolute top-full right-0 mt-2 w-56 bg-white border border-stone-100 shadow-xl rounded-xl overflow-hidden py-2"
                  >
                    <Link
                      to="/categories"
            ...
            <Link to="/about" className={`text-sm font-bold transition-colors hover:text-accent ${location.pathname === '/about' ? 'text-accent' : 'text-espresso'}`}>من نحن</Link>
            <Link to="/contact" className={`text-sm font-bold transition-colors hover:text-accent ${location.pathname === '/contact' ? 'text-accent' : 'text-espresso'}`}>تواصل معنا</Link>
          </nav>
          ...
          <form onSubmit={handleSearch} className="hidden lg:flex flex-grow max-w-md relative mx-4">
            <input 
              ...
              className="w-full bg-stone-100 border-none rounded-full py-2.5 pr-12 pl-4 text-sm focus:ring-2 focus:ring-accent outline-none"
            />
            <button type="submit" className="absolute right-4 top-1/2 -translate-y-1/2 text-stone-400 hover:text-accent">
              <Search size={18} />
            </button>
          </form>
        ...
        {isMenuOpen && (
          ...
          >
            <div className="px-4 pt-2 pb-6 space-y-1">
              <Link to="/" onClick={() => setIsMenuOpen(false)} className="block px-3 py-4 text-base font-bold text-espresso">الرئيسية</Link>
              <Link to="/about" onClick={() => setIsMenuOpen(false)} className="block px-3 py-4 text-base font-bold text-espresso">من نحن</Link>
              <Link to="/contact" onClick={() => setIsMenuOpen(false)} className="block px-3 py-4 text-base font-bold text-espresso">تواصل معنا</Link>
              <div className="px-3 py-2 text-xs font-bold text-stone-400 uppercase tracking-widest">المنتجات</div>
              {categories.map((cat) => {
                const theme = getCategoryTheme(cat);
                const imageUrl = getCategoryImage(cat);
                return (
